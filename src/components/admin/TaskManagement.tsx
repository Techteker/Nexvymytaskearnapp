            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4 pt-6 border-t border-white/[0.04]">
                 <div>
                    ...
                    <p className="text-xl font-black text-white italic tracking-tighter">{task.rewardCoins} <span className="text-[10px] text-purple-500 uppercase not-italic">Credits</span></p>
                 </div>
              ...
              <div className="absolute top-8 right-8 flex gap-2 opacity-0 group-hover:opacity-100 transition-all translate-y-[-10px] group-hover:translate-y-0">
                 <button 
                  onClick={() => { setEditingTask(task); setFormData({ ...task }); setModalOpen(true); }}
                 ...
                 <button 
                  onClick={async () => {
                    if (window.confirm('Terminate campaign? This action is IRREVERSIBLE.')) {
                      await adminService.deleteTask(task.$id);
                      showToast('Campaign terminated', 'success');
                      loadData();
                    }
                  }}
                  className="p-3 bg-rose-600 text-white rounded-2xl transition-all shadow-2xl"
                 >
                   <Trash2 size={18} />
                 </button>
              </div>
                            ...
                            <div className="w-32 h-32 bg-white/5 border border-white/10 border-dashed rounded-[2.5rem] flex flex-col items-center justify-center relative group overflow-hidden shrink-0 shadow-inner">
                               {formData.imageUrl ? (
                                  ...
                                  <>
                                    ...
                                    <span className="text-[8px] font-black text-slate-600 uppercase mt-2">LOGO</span>
                                  </>
                               )}
                               <input 
                                 type="file" 
                               ...
                               <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                  <span className="text-[10px] font-black text-white uppercase italic">Replace</span>
                               </div>
                            </div>
