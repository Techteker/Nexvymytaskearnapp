...
export const AffiliateManagement = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Handle file upload
      if (logoFile) {
        const uploadedUrl = await apiService.uploadFile(logoFile);
  ...
  const openModal = (type: 'partner' | 'offer' | 'category', item: any = null) => {
    ...
    setEditItem(item);
    setLogoFile(null);
    setLogoPreview(item?.logoUrl || item?.partnerLogo || null);
    setShowModal(true);
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {loading ? (
          ...
          filteredItems().map((item: any) => (
            <div 
              key={item.id} 
              className="gaming-card p-5 group flex flex-col gap-4 border-white/10 hover:border-emerald-500/30 transition-all"
            >
              <div className="flex items-start justify-between">
                ...
                <div className="flex gap-2">
                   ...
                   <button 
                     onClick={() => handleDelete(item.id, activeTab === 'partners' ? 'partner' : activeTab === 'offers' ? 'offer' : 'category')}
              ...
              <div>
                 <div className="flex items-center gap-2 mb-1">
                    <h4 className="text-white font-black text-sm uppercase leading-tight line-clamp-1">{item.name || item.title}</h4>
                    {item.isActive && <Check className="w-3 h-3 text-emerald-400" />}
                    {item.isTrending && <TrendingUp className="w-3 h-3 text-amber-400" />}
                 </div>
              ...
              {activeTab === 'offers' && (
                <div className="grid grid-cols-2 gap-2 mt-2">
                   <div className="bg-white/5 rounded-xl p-2 border border-white/5">
                      <p className="text-[8px] text-white/40 font-black uppercase mb-1">Reward</p>
                      <p className="text-xs font-black text-white">{item.rewardCoins} Coins</p>
                   </div>
                   <div className="bg-white/5 rounded-xl p-2 border border-white/5">
                      <p className="text-[8px] text-white/40 font-black uppercase mb-1">Cashback</p>
                      <p className="text-xs font-black text-white">{item.cashbackPercentage}%</p>
                   </div>
                </div>
              )}

              <button className="w-full mt-auto py-2 bg-white/5 border border-white/10 rounded-xl text-[10px] font-black text-white/60 uppercase group-hover:bg-emerald-500 group-hover:text-white transition-all">
                 View Analytics
              </button>
            </div>
          ))
                  ...
                  <>
                    <div className="space-y-2 col-span-2">
                      ...
                      <div className="flex items-center gap-6 p-4 bg-white/5 border border-white/10 rounded-2xl">
                        <div className="w-16 h-16 bg-white rounded-xl flex items-center justify-center p-2 shadow-inner overflow-hidden flex-shrink-0">
                          {logoPreview ? (
                        ...
                        <div className="flex-1 space-y-2">
                          <p className="text-[9px] font-bold text-white/30 uppercase tracking-widest">Upload square logo (PNG/JPG)</p>
                          <input 
                            type="file" 
                            accept="image/*" 
                            onChange={handleFileChange}
                            className="text-xs text-white/60 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-[10px] file:font-black file:uppercase file:bg-emerald-500/20 file:text-emerald-400 hover:file:bg-emerald-500/30 cursor-pointer"
                          />
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <input name="websiteUrl" defaultValue={editItem?.websiteUrl} required className="w-full bg-white/5 border border-white/10 rounded-2xl py-3 px-6 text-sm text-white font-bold focus:border-emerald-500/50" />
                    </div>
                    <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <input type="number" step="0.1" name="rating" defaultValue={editItem?.rating || 5.0} className="w-full bg-white/5 border border-white/10 rounded-2xl py-3 px-6 text-sm text-white font-bold focus:border-emerald-500/50" />
                    </div>
                  ...
                  <>
                    <div className="space-y-2 col-span-2">
                      ...
                      <div className="flex items-center gap-6 p-4 bg-white/5 border border-white/10 rounded-2xl">
                        ...
                        <div className="flex-1 space-y-2">
                          <p className="text-[9px] font-bold text-white/30 uppercase tracking-widest">Leave empty to use partner logo</p>
                          <input 
                            type="file" 
                            accept="image/*" 
                            onChange={handleFileChange}
