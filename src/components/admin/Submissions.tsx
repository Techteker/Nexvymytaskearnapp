                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                           <span className="text-xs font-black text-white uppercase italic tracking-tighter truncate max-w-[120px]">{subProofs(sub).username || sub.userId}</span>
                           <span className="text-[8px] font-bold text-slate-500 uppercase tracking-widest truncate max-w-[120px]">{subProofs(sub).email || 'Active ID'}</span>
                        </div>
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                           <span className="text-xs font-black text-white uppercase italic tracking-tighter truncate max-w-[120px]">{subProofs(sub).taskName || sub.taskId}</span>
               ...
               <div className="w-full md:w-[28rem] p-12 flex flex-col bg-[#020617]">
                  ...
                  <div className="space-y-4 mb-6 overflow-y-auto max-h-[40vh] pr-2 custom-scrollbar">
                     ...
                     <div className="p-4 bg-white/[0.03] border border-white/5 rounded-2xl">
                        <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest italic mb-1">Task Name</p>
                        <p className="text-xs font-black text-white">{subProofs(selectedSub).taskName || selectedSub.taskId}</p>
                     </div>

                     {/* Username Field */}
                     <div className="p-4 bg-white/[0.03] border border-white/5 rounded-2xl">
                        <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest italic mb-1">Operative Username</p>
                        <p className="text-xs font-black text-blue-400">{subProofs(selectedSub).username || selectedSub.userId}</p>
                     </div>

                     {/* Email/Gmail Field */}
                     <div className="p-4 bg-white/[0.03] border border-white/5 rounded-2xl">
                        <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest italic mb-1">Operative Gmail</p>
                        <p className="text-xs font-black text-emerald-400 select-all">{subProofs(selectedSub).email || 'No email sent'}</p>
                     </div>

                     {/* Completion Link Field */}
                     {subProofs(selectedSub).link && (
                        <div className="p-4 bg-white/[0.03] border border-white/5 rounded-2xl flex items-center justify-between gap-2 group border-blue-500/10 hover:border-blue-500/25 transition-all">
                           <div className="flex-1 min-w-0">
                              <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest italic mb-1">Evidence URL</p>
                              <p className="text-xs font-mono text-slate-300 truncate">{subProofs(selectedSub).link}</p>
                           </div>
                           <a 
                             href={subProofs(selectedSub).link}
                             target="_blank"
                             rel="noopener noreferrer"
                             className="p-2 bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 rounded-xl transition-all shrink-0"
                           >
                     ...
                     {/* Traditional Operative Hash ID */}
                     <div className="p-4 bg-white/[0.01] border border-white/5 rounded-2xl flex items-center justify-between">
                        <div>
                           <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest italic">Operative ID Hash</p>
                           <p className="text-[10px] font-mono text-slate-500 truncate max-w-[200px]">{selectedSub.userId}</p>
                        </div>
                     </div>
                  </div>
...
const subProofs = (sub: Submission) => {
  if (!sub) return { screenshotUrl: '', text: '', taskName: '', username: '', email: '', link: '' };
  try {
     const proofs = typeof sub.proofs === 'string' ? JSON.parse(sub.proofs) : (sub.proofs || {});
     return {
        screenshotUrl: proofs.screenshotUrl || '',
        text: proofs.text || '',
        taskName: proofs.taskName || proofs.taskTitle || '',
        username: proofs.username || '',
        email: proofs.email || proofs.userEmail || '',
        link: proofs.link || ''
     };
  } catch (e) {
     return { screenshotUrl: '', text: '', taskName: '', username: '', email: '', link: '' };
  }
};
