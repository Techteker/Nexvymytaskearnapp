...
export const QuizGame = () => {
  ...
  React.useEffect(() => {
    const loadQuiz = async () => {
      ...
      try {
        ...
        if (found) {
          setQuizData(found);
          let parsedQuestions = found.questions;
  ...
  if (loading) {
    return (
      <>
        <SEO 
          title="Trivia Quiz Earning: Answer Quizzes for Cash - Nexvy" 
          description="Play daily fun quizzes and earn money on Nexvy app. Challenge your general knowledge in diverse topics, answer correctly, and earn instant coin payouts."
          keywords="quiz earn money, trivia quizzes, quizzes for cash, learn and earn india, nexvy quiz, gk quiz question, win coins online, real money quiz app, brain game rewards"
        />
    ...
    return (
      <>
        ...
        <div className="text-center p-10 flex flex-col items-center gap-4">
          ...
          <button onClick={() => navigate('/survey')} className="gaming-button-yellow px-6 py-2">Go Back</button>
        </div>
      </>
    );
  }
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between gap-4">
        <button onClick={() => navigate('/survey')} className="text-white/40 font-bold uppercase text-xs">Quit</button>
