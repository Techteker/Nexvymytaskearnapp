...
const users = [
  { rank: 1, name: 'DragonSlayer', coins: 142050, level: 45, status: 'online', change: 'up' },
  { rank: 2, name: 'Alice_Wonder', coins: 128400, level: 42, status: 'online', change: 'down' },
  { rank: 3, name: 'CyberKnight', coins: 115200, level: 38, status: 'offline', change: 'none' },
  { rank: 4, name: 'ProGamer', coins: 98400, level: 35, status: 'online', change: 'up', isSelf: true },
  { rank: 5, name: 'NightOwl', coins: 87200, level: 33, status: 'online', change: 'up' },
  { rank: 6, name: 'ShadowBoxer', coins: 76500, level: 31, status: 'offline', change: 'down' },
  { rank: 7, name: 'GoldDigger', coins: 65400, level: 29, status: 'online', change: 'none' },
];
        ...
        <div className="flex flex-col items-center flex-1 max-w-[100px]">
          <div className="relative mb-2">
            <div className="w-16 h-16 rounded-2xl bg-slate-400 p-0.5 border-2 border-slate-300">
              <img src="https://api.dicebear.com/7.x/avataaars/svg?seed=Alice" alt="" className="w-full h-full rounded-xl object-cover" />
            </div>
          ...
          <div className="w-full h-24 bg-gradient-to-t from-slate-600/40 to-slate-400/20 rounded-t-2xl border-t border-x border-white/10 flex flex-col items-center justify-center p-2 text-center">
             <span className="text-[10px] font-bold truncate w-full">Alice_Wonder</span>
             <div className="flex items-center gap-1 mt-1">
                <CoinIcon size={12} />
                <span className="text-[10px] font-black">128K</span>
             </div>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center flex-1 max-w-[120px] -translate-y-4">
          <div className="relative mb-2">
            <div className="w-20 h-20 rounded-2xl bg-yellow-400 p-0.5 border-2 border-yellow-200 shadow-[0_0_20px_rgba(251,191,36,0.3)]">
              <img src="https://api.dicebear.com/7.x/avataaars/svg?seed=Dragon" alt="" className="w-full h-full rounded-xl object-cover" />
            </div>
          ...
          <div className="w-full h-32 bg-gradient-to-t from-yellow-600/40 to-yellow-400/20 rounded-t-2xl border-t border-x border-yellow-400/30 flex flex-col items-center justify-center p-2 text-center shadow-[0_0_30px_rgba(251,191,36,0.1)]">
             <span className="text-xs font-black truncate w-full flex items-center justify-center gap-1">
               DragonSlayer <CheckCircle2 className="w-3 h-3 text-blue-400 fill-blue-400/20" />
             </span>
             <div className="flex items-center gap-1 mt-2">
                <CoinIcon size={14} />
                <span className="text-sm font-black text-white">142K</span>
             </div>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center flex-1 max-w-[100px]">
          <div className="relative mb-2">
            <div className="w-16 h-16 rounded-2xl bg-orange-700 p-0.5 border-2 border-orange-500">
              <img src="https://api.dicebear.com/7.x/avataaars/svg?seed=Cyber" alt="" className="w-full h-full rounded-xl object-cover" />
            </div>
          ...
          <div className="w-full h-20 bg-gradient-to-t from-orange-800/40 to-orange-700/20 rounded-t-2xl border-t border-x border-white/10 flex flex-col items-center justify-center p-2 text-center">
             <span className="text-[10px] font-bold truncate w-full">CyberKnight</span>
             <div className="flex items-center gap-1 mt-1">
                <CoinIcon size={12} />
                <span className="text-[10px] font-black">115K</span>
             </div>
          </div>
        </div>
      </div>
        ...
        {users.map((user) => (
          <motion.div
            ...
            className={`gaming-card p-4 flex items-center justify-between border-white/10 ${
              user.isSelf ? 'bg-gaming-accent/20 border-gaming-accent/40 ring-1 ring-gaming-accent/20 shadow-xl' : 'bg-blue-900/40'
            }`}
          >
            <div className="flex items-center gap-4">
              <div className="w-8 flex flex-col items-center gap-1">
                <span className={`text-[12px] font-black ${user.rank <= 3 ? 'text-white' : 'text-white/40'}`}>#{user.rank}</span>
                <RankIndicator change={user.change} />
              </div>
              
              <div className="relative">
                <div className={`w-12 h-12 rounded-xl overflow-hidden border-2 ${
                  user.isSelf ? 'border-gaming-accent' : 'border-white/10'
                }`}>
              ...
              <div>
                <h4 className="text-sm font-black text-white flex items-center gap-1">
                  {user.name}
                  {user.isSelf && <span className="text-[8px] bg-gaming-accent px-1.5 py-0.5 rounded uppercase">You</span>}
                </h4>
                <div className="flex items-center gap-2 mt-1">
                  <div className="h-1 w-12 bg-white/10 rounded-full overflow-hidden">
                    <div className="h-full bg-gaming-accent" style={{ width: '60%' }} />
                  </div>
                  <span className="text-[9px] font-bold text-white/40 uppercase">Lv.{user.level}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col items-end gap-1">
              <div className="flex items-center gap-1 bg-white/5 px-2 py-1 rounded-lg border border-white/10">
                <CoinIcon size={12} />
                <span className="text-xs font-black text-white">{user.coins.toLocaleString()}</span>
              </div>
            </div>
          </motion.div>
        ))}
